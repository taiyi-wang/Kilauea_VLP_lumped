...
def calcol_alyt(consts, plot = 'NO'):
	...
	Specialized to plane strain case
	Taiyi Wang Nov.22, 2021

	features in addition to Kumagai's model:
	1. magma inertia 2. zero magma outflow during collapse
	...
	# Unpack constants
	rho_p = consts[0]; eff_rho_m = consts[1]; L = consts[2]; g = consts[3]; w = consts[4];
	beta = consts[5]; dtau = consts[6]; H = consts[7]; T = consts[8];
	pi = np.pi

	# Characteristic quantities
	l_star = beta * H * (rho_p*L + eff_rho_m * H)*g
	t_star = np.sqrt(beta * H * (rho_p*L + eff_rho_m * H))
	p_star = (L * rho_p+ eff_rho_m * H) * g

	# Dimensionless results
	t_max_hat = pi
	t_hat = np.arange(0, t_max_hat, 0.01)

	pi_0 = 2*L/(w*g*(rho_p*L + eff_rho_m*H))*dtau
